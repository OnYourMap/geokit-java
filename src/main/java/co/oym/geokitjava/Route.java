...
public class Route {
	...
	public static class Utility {
		...
		private static Map<String, Resources> translationsByLang = new HashMap<>();

		public static void loadTranslation(String lang, InputStream resourceInputStream) throws Exception {
			Resources res = new Resources(resourceInputStream);
			translationsByLang.put(lang, res);
		}
		
		public static Resources getResources(String lang) {
			return translationsByLang.get(lang);
		}
		...
		public static class Resources {
			...
			private static JacksonXmlModule module;
			...
			public Resources(InputStream is) throws Exception {
				//Resources res = xmlMapper.readValue(is, Resources.class);
				POJO pojo = xmlMapper.readValue(is, POJO.class);
				for (KV kv : pojo.strings) {
					strings.put(kv.name.replaceAll("\\.", "\\_"), StringEscapeUtils.unescapeJava(kv.value));
				}
				System.out.println(strings);
			}
			
			public String getString(R.string key) {
				return strings.get(key.toString());
			}
		...
		public static String renderInstruction(Instruction i, Resources res) {
			if (res == null)
		...
		private static String rebuild(Resources res, R.string resId, final Object[] args) {
			final String template = res.getString(resId);
			return MessageFormat.format(template, args);
		}

		private static String decodeHeading2(String manHeading, Resources res) {
			if (manHeading != null) {
		...
		private static String decodeTransportType2(String ptType, Resources res) {
			if (ptType != null) {
		...
		private static String renderTime2(String time, Resources res) {
			String t = "";
...
//		private static String decodeHeading(String manHeading, Resource res) {
//			if (manHeading != null) {
